...
import { 
  ...
  Code, Clock, CheckCircle, XCircle, BarChart2, Gauge, Eye, Hash
} from 'lucide-react';
...
const LiveInterviewPage = () => {
  ...
  const [code, setCode] = useState(`function findDuplicate(nums) {\n  // Your solution here\n}`);
  ...
  const aiStats = {
    typingSpeed: 'Fast 🟢',
    logicScore: '8/10',
    hesitation: 'Paused for 15s before recursion',
    plagiarism: '100% original ✅',
    behavior: [
      'Used nested loop approach initially',
      'Optimized to hash map after suggestion',
  ...
  const problem = {
    ...
    description: 'Optimize a function that finds duplicate numbers in an array in O(n) time complexity.',
    example: {
      input: '[1, 3, 4, 2, 2]',
      output: '2'
    },
    constraints: [
      ...
      'Array contains only one duplicate'
    ]
  };

  // AI suggestions for recruiter
  const aiSuggestions = [
    'Candidate used a nested loop approach; suggest an optimized hashing solution',
    'Candidate hesitated for 25s before implementing recursion - possible knowledge gap?',
    'Typing speed slowed when dealing with edge cases - ask about test case coverage'
  ];

  const aiQuestions = [
    'Can you optimize this to O(n) using a hash set?',
    'How would you modify this to work for streams of infinite data?',
        ...
        <div className="w-1/4 border-r border-gray-700 bg-black/30 backdrop-blur-lg overflow-auto">
          <div className="p-4">
            <h2 className="text-lg font-medium text-white flex items-center">
              ...
              Live Coding Interview with Recruiter
            </h2>
            ...
            <div className="mt-6">
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-sm font-medium text-green-400">{aiStats.typingSpeed}</span>
                </div>

                <div className="flex justify-between">
                  <span className="text-sm text-gray-400">Logical Thinking</span>
                  <span className="text-sm font-medium text-blue-400">{aiStats.logicScore}</span>
                </div>

                <div className="flex justify-between">
                  <span className="text-sm text-gray-400">Hesitation Analysis</span>
                  <span className="text-sm font-medium text-yellow-400">{aiStats.hesitation}</span>
                </div>

                <div className="flex justify-between">
                  <span className="text-sm text-gray-400">Plagiarism Score</span>
                  <span className="text-sm font-medium text-green-400">{aiStats.plagiarism}</span>
                </div>
              </div>
              ...
              <div className="mt-4">
                ...
                <ul className="space-y-1 text-xs text-gray-300">
                  {aiStats.behavior.map((item, index) => (
          ...
          <div className="flex-grow relative">
            ...
            <div className="absolute bottom-4 right-4 w-64 bg-blue-500/10 p-2 rounded-md shadow-lg border border-blue-500/30">
              <div className="flex">
                ...
                <div className="ml-2">
                  <p className="text-xs font-medium text-blue-300">AI Hint:</p>
                  <p className="text-xs text-blue-400">Consider using a hash set for O(n) time complexity</p>
                </div>
              </div>
            </div>
          </div>
